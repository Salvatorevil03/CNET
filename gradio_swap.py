...
CONTROLNET_WEIGHTS = {
    ...
    "MLSD (Hough Lines)": './models/control_sd15_mlsd.pth'
}
...
def process_word_swap(input_image, source_prompt, target_prompt, control_type, tau, num_samples, 
    ...
    with torch.no_grad():
        # MODIFICA 1: Forziamo l'immagine in un quadrato esatto per allinearla ai tensori della Null-Text
        img = cv2.resize(HWC3(input_image), (image_resolution, image_resolution), interpolation=cv2.INTER_LANCZOS4)
        H, W, C = img.shape 
        
        # Calcolo dinamico dell'area attesa per le mappe di attenzione (riduzione 32x)
        attention_mod.EXPECTED_SHAPE = (H // 32) * (W // 32)
        
        # L'immagine full ora è un quadrato, così i detector non sfalsano le dimensioni (64x64)
        input_image_full = img.copy() 
        ...
        elif control_type == "MLSD (Hough Lines)":
        ...
        un_cond = {"c_concat": None if guess_mode else [control], "c_crossattn": [uncond_embeddings]}
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            # MODIFICA 3: Messo OpenPose come valore di default al posto di Canny per prevenire errori
            control_type = gr.Dropdown(
                label="Tipologia ControlNet", 
                choices=[
                    "Canny", "Depth", "Normal", "OpenPose", 
                    "Segmentation", "HED", "Fake Scribble", 
                    "Scribble (Da Immagine/Bozza)", "MLSD (Hough Lines)", "Nessuno (SD Base)"
                ], 
                value="OpenPose"
            )

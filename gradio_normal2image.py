...
def process(input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold):
    ...
    return [detected_map] + results, inter_images, inter_images_noise, top_row_maps, bottom_row_maps

block = gr.Blocks().queue()
with block:
    with gr.Row(): gr.Markdown("## Control Stable Diffusion with Normal Maps & Attention Viz")
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(sources=['upload'], type="numpy")
            prompt = gr.Textbox(label="Prompt")
            target_word = gr.Textbox(label="Word to track (e.g. 'object')", value="") 
    ...
    ips = [input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, bg_threshold]

...
def process(input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
...
with block:
    with gr.Row(): gr.Markdown("## Control Stable Diffusion with Segmentation Maps & Attention Viz")
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(sources=['upload'], type="numpy")
            prompt = gr.Textbox(label="Prompt")
            target_word = gr.Textbox(label="Word to track (e.g. 'sky')", value="") 
    ...
    ips = [input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta]

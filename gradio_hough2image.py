...
def process(input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, value_threshold, distance_threshold):
    attention_mod.STORE_ATTN.clear()
    with torch.no_grad():
        img_resized = resize_image(HWC3(input_image), image_resolution)
    ...
    return [255 - cv2.dilate(detected_map, np.ones(shape=(3, 3), dtype=np.uint8), iterations=1)] + results, inter_images, inter_images_noise, top_row_maps, bottom_row_maps

block = gr.Blocks().queue()
with block:
    with gr.Row(): gr.Markdown("## Control Stable Diffusion with Hough Line Maps & Attention Viz")
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(sources=['upload'], type="numpy")
            prompt = gr.Textbox(label="Prompt")
            target_word = gr.Textbox(label="Word to track (e.g. 'building')", value="") 
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                value_threshold = gr.Slider(label="Hough threshold", minimum=0.01, maximum=2.0, value=0.1, step=0.01)
                distance_threshold = gr.Slider(label="Hough dist threshold", minimum=0.01, maximum=20.0, value=0.1, step=0.01)
    ...
    ips = [input_image, prompt, a_prompt, n_prompt, target_word, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, value_threshold, distance_threshold]
